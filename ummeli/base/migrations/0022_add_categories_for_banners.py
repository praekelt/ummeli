...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        if not Category.objects.filter(slug='vlive-banner').exists():
            category = Category.objects.create(title='Vlive Banner',
                                               subtitle='Vlive Banner',
                                               slug='vlive-banner',
                                               )
            category.sites.add(1)

        if not Category.objects.filter(slug='homepage-banner').exists():
            category = Category.objects.create(title='Homepage Banner',
    ...
    def backwards(self, orm):
        Category = orm['category.Category']
        if Category.objects.filter(slug='vlive-banner').exists():
            Category.objects.get(slug='vlive-banner').delete()
        if Category.objects.filter(slug='homepage-banner').exists():
            Category.objects.get(slug='homepage-banner').delete()
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 11, 8, 14, 30, 8, 467436)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 11, 8, 14, 30, 8, 467355)'}),
